...
/*
    ...
    public List<DetallePlanilla> findPlaDetalles(PlanillaPK planillaPK) {
        List<Object[]> det = null;
        List<DetallePlanilla> listaDeta = new ArrayList<DetallePlanilla>(0);
        Integer cia = 1;

        Query dt = em.createQuery("select distinct D.planilla, P from DetPlanilla D, PuestoEmpleado P where D.empleado.empleadoPK = P.empleado.empleadoPK and D.detPlanillaPK.idCompania = :codCia and D.detPlanillaPK.numPlanilla = :pla  and D.detPlanillaPK.anio = :anio  and D.detPlanillaPK.mes = :mes");
        dt.setParameter("codCia", 1);
        dt.setParameter("pla", planillaPK.getNumPlanilla());
        dt.setParameter("anio", planillaPK.getAnio());
        dt.setParameter("mes", planillaPK.getMes());
        ...
        for (Object[] o : det) {
            ...
            Query pres = em.createQuery("select sum (det.monto)  from DetPlanilla det, DeduccionesPrestaciones ded  where ded.tipo = 'S'  and ded.deduccionesPrestacionesPK = det.deduccionesPrestaciones.deduccionesPrestacionesPK  and det.detPlanillaPK.numPlanilla = :pla  and det.detPlanillaPK.idCompania = :cia  and det.detPlanillaPK.idEmpleado = :emp");
            ...
            Query ded = em.createQuery("select sum (deta.monto)  from DetPlanilla deta, DeduccionesPrestaciones deduc  where deduc.tipo = 'R'  and deduc.deduccionesPrestacionesPK = deta.deduccionesPrestaciones.deduccionesPrestacionesPK  and deta.detPlanillaPK.numPlanilla = :pla  and deta.detPlanillaPK.idCompania = :cia  and deta.detPlanillaPK.idEmpleado = :emp");
            ...
            Query ing = em.createQuery("select dp from DetPlanilla dp, DeduccionesPrestaciones ded  where ded.tipo = 'S'  and dp.deduccionesPrestaciones.deduccionesPrestacionesPK = ded.deduccionesPrestacionesPK  and dp.detPlanillaPK.idCompania = :cia  and dp.detPlanillaPK.anio = :anio  and dp.detPlanillaPK.mes = :mes  and dp.detPlanillaPK.numPlanilla = :pla  and dp.detPlanillaPK.idEmpleado = :emp ", DetPlanilla.class);
...
//    List<Object[]> listEgreso = null;
            ...
            Query egr = em.createQuery("select dp from DetPlanilla dp, DeduccionesPrestaciones ded  where ded.tipo = 'R'  and dp.deduccionesPrestaciones.deduccionesPrestacionesPK = ded.deduccionesPrestacionesPK  and dp.detPlanillaPK.idCompania = :cia  and dp.detPlanillaPK.anio = :anio  and dp.detPlanillaPK.mes = :mes  and dp.detPlanillaPK.numPlanilla = :pla  and dp.detPlanillaPK.idEmpleado = :emp ", DetPlanilla.class);
